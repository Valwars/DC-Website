    ...
    if(!empty($_POST['name']) && !empty($_POST['password'])) // Si il existe les champs email, password et qu'il sont pas vident
    {
        ...
        if($row > 0)
        {
                ...
                if(password_verify($password, $data['mdp']))
                {
                    ...
                    die();
                }else{ header('Location: ../Francais/administrationHome.php?login_err=password'); die(); }
            
        }else{ header('Location: ../Francais/administrationHome.php?login_err=already'); die(); }
    }else{ header('Location: ../Francais/administrationHome.php?'); die();} // si le formulaire est envoyé sans aucune données
